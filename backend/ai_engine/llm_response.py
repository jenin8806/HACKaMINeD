...
{{
  "episodes": [
    {{
      ...
      "improvements": {{
        "what": "One sentence: the single most impactful change to make to this episode",
        "why": "One sentence: exactly why this change will increase audience retention",
...
def call_llm_apply_changes(episodes: list) -> dict | None:
    ...
    # Build a compact representation for the prompt — include title, segments, improvements
    compact = []
    for ep in episodes:
        compact.append({
...
def call_llm_chat(user_message: str, episodes_context: list) -> str | None:
    ...
    if episodes_context:
        # Slim down context to avoid token bloat
        slim = []
