...
const DEFAULT_EFFECT_OPTIONS: Required<HyperspeedOptions> = {
  ...
  colors: {
    ...
    // lime/green tones matching TheVbox brand
    leftCars: [0xc7f711, 0x8cb535, 0xa9f42c],
    // cool blue-teal tones matching the dark backdrop
    rightCars: [0x1a8a6e, 0x0e5ea5, 0x2a5a52],
    sticks: 0xc7f711,
  },
};
  ...
  return {
    mountainDistortion: {
      ...
      getJS: (progress: number, time: number) => {
        ...
        const d = new THREE.Vector3(
          Math.cos(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.cos(f * Math.PI * uFreq.x + time) * uAmp.x,
          nsin(progress * Math.PI * uFreq.y + time) * uAmp.y - nsin(f * Math.PI * uFreq.y + time) * uAmp.y,
          nsin(progress * Math.PI * uFreq.z + time) * uAmp.z - nsin(f * Math.PI * uFreq.z + time) * uAmp.z,
        );
    ...
    xyDistortion: {
      ...
      getJS: (progress: number, time: number) => {
        ...
        const d = new THREE.Vector3(
          Math.cos(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.cos(f * Math.PI * uFreq.x + time) * uAmp.x,
          Math.sin(progress * Math.PI * uFreq.y + time + Math.PI / 2) * uAmp.y - Math.sin(f * Math.PI * uFreq.y + time + Math.PI / 2) * uAmp.y,
          0,
        );
    ...
    LongRaceDistortion: {
      ...
      getJS: (progress: number, time: number) => {
        ...
        const d = new THREE.Vector3(
          Math.sin(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.sin(f * Math.PI * uFreq.x + time) * uAmp.x,
          Math.sin(progress * Math.PI * uFreq.y + time) * uAmp.y - Math.sin(f * Math.PI * uFreq.y + time) * uAmp.y,
          0,
        );
...
const sideSticksVertex = `
  ...
  void main(){
    ...
    float t=mod(uTime*60.*2.+aOffset,uTravelLength);
    transformed=(rotationY(3.14/2.)*vec4(transformed,1.)).xyz;
    transformed.z+=-uTravelLength+t;
    float progress=abs(transformed.z/uTravelLength);
    transformed.xyz+=getDistortion(progress);
...
type FullOptions = Required<HyperspeedOptions> & {
  colors: Required<ColorOptions>;
  distortionObj: ReturnType<typeof buildDistortions>[keyof ReturnType<typeof buildDistortions>];
};
...
class CarLights {
  ...
  constructor(webgl: AppInstance, options: FullOptions, colors: number[], speed: [number, number], fade: THREE.Vector2) {
    this.webgl = webgl;
  ...
  init() {
    ...
    let cols: THREE.Color[] = this.colors.map((c) => new THREE.Color(c));
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: Object.assign(
        ...
        options.distortionObj.uniforms,
      ),
    });
    material.onBeforeCompile = (shader) => {
      shader.vertexShader = shader.vertexShader.replace('#include <getDistortion_vertex>', options.distortionObj.getDistortion);
    };
...
class LightsSticks {
  ...
  constructor(webgl: AppInstance, options: FullOptions) {
    this.webgl = webgl;
    this.options = options;
  }
...
class Road {
  ...
  createPlane(side: number, _width: number, isRoad: boolean) {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      side: THREE.DoubleSide,
      uniforms: Object.assign(uniforms, this.webgl.fogUniforms, options.distortionObj.uniforms),
    });
    material.onBeforeCompile = (shader) => {
      shader.vertexShader = shader.vertexShader.replace('#include <getDistortion_vertex>', options.distortionObj.getDistortion);
    };
...
class App implements AppInstance {
  ...
  constructor(container: HTMLElement, options: FullOptions) {
    this.options = options;
  ...
  initPasses() {
    ...
    this.bloomPass = new EffectPass(this.camera, new BloomEffect({ luminanceThreshold: 0.2, luminanceSmoothing: 0, resolutionScale: 1 } as any));
  ...
  onMouseDown(ev: MouseEvent) {
    this.options.onSpeedUp(ev as unknown as Event);
    this.fovTarget = this.options.fovSpeedUp;
    this.speedUpTarget = this.options.speedUp;
  }
  onMouseUp(ev: MouseEvent) {
    this.options.onSlowDown(ev as unknown as Event);
  ...
  update(delta: number) {
    ...
    const dist = this.options.distortionObj;
    if ('getJS' in dist && dist.getJS) {
      const distortion = dist.getJS(0.025, time);
...
const Hyperspeed = ({ effectOptions = {} }: Props) => {
  ...
  useEffect(() => {
    ...
    const distKey = (merged.distortion as keyof typeof distortions) in distortions
      ? (merged.distortion as keyof typeof distortions)
      : 'turbulentDistortion';

    const fullOptions = {
      ...merged,
      distortionObj: distortions[distKey],
    } as FullOptions;

    const app = new App(container, fullOptions);
    appRef.current = app;
