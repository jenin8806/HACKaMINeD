...
export function UserSettingsDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  const { user, saveProfile, changePassword } = useUser();
  ...
  }, [open, user.username, user.email, user.profilePic]);

  const handleSaveProfile = async () => {
    setProfileError("");
    setSavingProfile(true);
    const result = await saveProfile({ username, email, profilePic });
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full h-10 px-3 rounded-xl bg-[#1A262E] border border-white/10 text-sm text-[#E8E9E8] placeholder:text-[#E8E9E8]/30 focus:outline-none focus:border-[#C7F711]/50 transition-all"
                  />
                </div>
                <div className="space-y-1.5">
                ...
                <button
                  ...
                  className="w-full py-2.5 rounded-xl font-semibold text-sm bg-[#C7F711] text-[#0E1921] hover:bg-[#A9F42C] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
